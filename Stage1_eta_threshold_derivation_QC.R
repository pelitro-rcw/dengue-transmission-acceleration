...
OUT_DIR <- file.path(getwd(), "Stage1_eta_thresholds_QC")
...
if (!is.null(roc_m2)) {
  ...
  panel_b <- ggplot2::ggplot() +
    ggplot2::annotate("text", x = 0.5, y = 0.5,
                      label = "ROC unavailable", size = 4) +
    ggplot2::xlim(0, 1) + ggplot2::ylim(0, 1) +
    ggplot2::labs(title = "b    ROC curve of STA/LTA ratio") +
    theme_pub(9)
}
...
save_figure(
  "Fig_eta_threshold_derivation_QC.pdf",
  fig_combined,
  width  = 14, height = 10,
...
save_figure(
  "Fig_eta_threshold_derivation_QC.png",
  fig_combined,
  width  = 14, height = 10,
  dpi    = 600, device = "png"
)
