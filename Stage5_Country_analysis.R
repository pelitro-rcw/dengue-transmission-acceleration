...
#   Stage5_Figure6_Legend_Drafts.txt
#   Stage5_Figure7_Legend_Draft.txt
#
# FRAMEWORK
# ---------
...
OUTPUT_DIR <- file.path(getwd(), "Stage5_Country_analysis")
...
# ------------------------------------------------------------------------------
compute_detector_paired_wilcoxon <- function(metric_id, country_metrics_df) {
...
build_country_multipanel <- function(metric_id) {
  ...
  (top / legend_strip / bottom) +
    ...
    patchwork::plot_annotation(
      title = paste0("Figure 6 | Country generalisability: ",
...
build_method_summary_matrix <- function() {
  ...
  ggplot2::ggplot(df) +
    ...
    ggplot2::coord_cartesian(clip = "off") +
    ggplot2::labs(
      title = "a    Country dominance matrix on 5 operational metrics",
      x = NULL, y = NULL
    ) +
    theme_dashboard(base_size = 8.5, base_family = base_family_global) +
    ggplot2::theme(
...
for (m in HH_METRICS_COUNTRY) {
  ...
  pdf_out <- file.path(OUTPUT_DIR, paste0("Figure6_", slug, ".pdf"))
  png_out <- file.path(OUTPUT_DIR, paste0("Figure6_", slug, ".png"))
  ggplot2::ggsave(filename = pdf_out, plot = fig, device = safe_pdf_device(),
                  width = fig6_w, height = fig6_h,
...
cat("\n=== Building Method-Across-Metrics Summary figure (Figure 6 set) ===\n")
{
  ...
  pdf_out <- file.path(OUTPUT_DIR, "Figure6_Method_Summary.pdf")
  png_out <- file.path(OUTPUT_DIR, "Figure6_Method_Summary.png")
...
winner_rows <- winner_rows %>%
  dplyr::mutate(
    ...
    # `Sig.` column shown in Figure 7 Panel A column 4: one-sample binomial
...
p_table <- ggplot2::ggplot() +
  ...
  ggplot2::labs(
    title = "a   Bootstrap-winning detector and its per-metric significance, by country"
  ) +
...
panel_b_long <- dominance_df_canonical %>%
  dplyr::select(COUNTRY, P_ConstantTA, P_ContinuousTA, P_OutbreakThreshold) %>%
...
# Diagnostic banner.
cat("\n=== Panel B: per-detector dot plot data (",
    nrow(panel_b_long), " dots = ", n_countries,
    " countries x 3 detectors) ===\n", sep = "")
...
p_dot <- ggplot2::ggplot(panel_b_long,
  ...
  # Per-country tier-legend overlay (top-right corner)
  ...
  ggplot2::annotate("text",
                    x = 3.58, y = 1.01,
                    label = "Per-country tier (dot border)",
                    hjust = 0, vjust = 1, size = 3.00, fontface = "bold",
                    family = base_family_global, colour = "grey15") +
  ggplot2::annotate("text",
                    x = 3.58, y = 0.97,
                    label = sprintf(
                      "(%d strong, %d partial, %d lead_only, %d contested)",
                      n_strong, n_partial, n_lead_only, n_contested),
                    hjust = 0, vjust = 1, size = 2.50, fontface = "italic",
                    family = base_family_global, colour = "grey35") +
  # Strong leader: thick black border
  ggplot2::annotate("point", x = 3.62, y = 0.93,
                    shape = 21, fill = "grey80",
                    colour = "#1F1F1F", stroke = 0.95, size = 3.2) +
  ggplot2::annotate("text", x = 3.68, y = 0.93,
                    label = "strong (both pairs sig)",
                    hjust = 0, vjust = 0.5, size = 2.65,
                    family = base_family_global, colour = "grey15") +
  ggplot2::annotate("point", x = 3.62, y = 0.89,
                    shape = 21, fill = "grey80",
                    colour = "#1F1F1F", stroke = 0.65, size = 3.2) +
  ggplot2::annotate("text", x = 3.68, y = 0.89,
                    label = "partial (one pair sig)",
                    hjust = 0, vjust = 0.5, size = 2.65,
                    family = base_family_global, colour = "grey15") +
  ggplot2::annotate("point", x = 3.62, y = 0.85,
                    shape = 21, fill = "grey80",
                    colour = "grey45", stroke = 0.55, size = 3.2) +
  ggplot2::annotate("text", x = 3.68, y = 0.85,
                    label = "lead only (no sig support)",
                    hjust = 0, vjust = 0.5, size = 2.65,
                    family = base_family_global, colour = "grey25") +
  ggplot2::annotate("point", x = 3.62, y = 0.81,
                    shape = 21, fill = "grey80",
                    colour = "grey55", stroke = 0.30, size = 3.2) +
  ggplot2::annotate("text", x = 3.68, y = 0.81,
                    label = "non-leader / contested",
                    hjust = 0, vjust = 0.5, size = 2.65,
                    family = base_family_global, colour = "grey45") +
  ...
  ggplot2::scale_x_discrete(
    labels = c("Constant TA"        = "Constant\nTA",
               ...
               "Outbreak Threshold" = "Outbreak\nThreshold"),
    expand = ggplot2::expansion(add = c(0.5, 1.8))
  ) +
  ggplot2::labs(
    title = "b   Per-detector country dominance probabilities",
    x     = NULL,
...
# ------------------------------------------------------------------------------
fig7 <- (p_table / p_dot) +
  patchwork::plot_layout(heights = c(0.55, 1.00)) +
  patchwork::plot_annotation(
    title = "Figure 7 | Bootstrap-supported country dominance of three target detectors",
    theme = ggplot2::theme(
...
FIG7_W <- 12
FIG7_H <- 11
ggplot2::ggsave(file.path(OUTPUT_DIR, "Figure7_Country_Summary.pdf"),
                fig7, width = FIG7_W, height = FIG7_H,
                device = safe_pdf_device(),
                units = "in", limitsize = FALSE, bg = "white")
ggplot2::ggsave(file.path(OUTPUT_DIR, "Figure7_Country_Summary.png"),
                fig7, width = FIG7_W, height = FIG7_H, dpi = 600,
                device = "png",
                units = "in", limitsize = FALSE, bg = "white")
cat("\nSaved: ", file.path(OUTPUT_DIR, "Figure7_Country_Summary.pdf"), "\n",
    sep = "")
cat("Saved: ", file.path(OUTPUT_DIR, "Figure7_Country_Summary.png"), "\n",
    sep = "")
...
#   Stage5_Figure6_Legend_Drafts.txt   (one block per metric + method summary)
#   Stage5_Figure7_Legend_Draft.txt    (single block for the country summary figure)
# ------------------------------------------------------------------------------
...
# Combined Figure 6 legend draft (one block per metric + Method Summary).
fig6_legend_lines <- c(
  paste0("================================================================"),
...
writeLines(fig6_legend_text,
           file.path(OUTPUT_DIR, "Stage5_Figure6_Legend_Drafts.txt"))
cat("\nSaved: ", file.path(OUTPUT_DIR, "Stage5_Figure6_Legend_Drafts.txt"),
    "\n", sep = "")

# Figure 7 legend draft
build_figure7_legend <- function() {
  ...
  paste0(
    ...
    "Taiwan). The figure has two panels: (a) a per-country metric table ",
    "with embedded per-metric significance, and (b) a per-detector dot plot ",
    "of country dominance probabilities with a per-country tier-legend ",
    "overlay. The per-country per-metric significance results are also ",
    "exported as a standalone CSV (Stage5_Country_Summary_PanelD_HeatmapData.csv) ",
...
writeLines(fig7_legend_text,
           file.path(OUTPUT_DIR, "Stage5_Figure7_Legend_Draft.txt"))
cat("Saved: ", file.path(OUTPUT_DIR, "Stage5_Figure7_Legend_Draft.txt"),
    "\n", sep = "")
...
cat("  Figure7_Country_Summary.{pdf,png}\n")
...
cat("Legend drafts:\n")
cat("  Stage5_Figure6_Legend_Drafts.txt\n")
cat("  Stage5_Figure7_Legend_Draft.txt\n")
