...
#   Stage4_Figure4_Legend_Drafts.txt
#   Stage4_Figure5_Legend_Draft.txt
#
# FRAMEWORK
# ---------
...
for (pkg in optional_geom_packages) {
  if (!pkg %in% installed_pkgs) {
    try(install.packages(pkg, dependencies = TRUE,
...
# Cairo PDF device with a base-pdf fallback for systems without cairo.
...
INPUT_PATH <- "C:/Users/User/Desktop/Dengue-Rainfall_Dataset.xlsx"
# INPUT_PATH <- file.path(getwd(), "Dengue-Rainfall_Dataset.xlsx")
# INPUT_PATH <- file.choose()

SHEET_NAME <- "Regional Data"

# Output directory. Created if absent.
OUTPUT_DIR <- file.path(getwd(), "Stage4_Regional_analysis")
if (!dir.exists(OUTPUT_DIR)) dir.create(OUTPUT_DIR, recursive = TRUE)

# Optional path to a Philippine administrative shapefile for the Figure 5
# choropleth. Leave NULL to use geodata (GADM) or rnaturalearth as the
# geometry source.
PH_SHAPEFILE <- NULL

# Default font: Arial > Helvetica > sans
available_fonts <- names(grDevices::pdfFonts())
base_family_global <- if ("Arial" %in% available_fonts) {
  "Arial"
...
save_plot_file <- function(filename, plot_obj, width, height,
                           dpi = 600, device = NULL) {
  ggplot2::ggsave(
...
# Continuous TA (3-week / 12-week ratio)
ETA_ON_CLASSIC <- 1.33
...
method_type_map <- tibble::tibble(
  ...
  Paradigm = c(
    "Acceleration Measures",    # Continuous TA
    "Acceleration Measures",    # Constant TA
    "Acceleration Measures",    # Incidence Gradient
    "Acceleration Measures",    # Hydrological Inflection
    "Retrospective Thresholds", # Alarm Threshold
    "Acceleration Measures",    # Composite Outbreak Signal
    "Retrospective Thresholds", # Outbreak Threshold
    "National Standard",        # WHO 90th
    "Acceleration Measures",    # Critical Transition
    "Acceleration Measures",    # CUSUM
    "National Standard"         # WHO 75th
  )
)
...
for (reg in names(boot_results)) {
  ...
  dominance_results[[reg]] <- data.frame(
    ...
    P_ConstantTA   = as.numeric(win_tab["Constant Transmission Acceleration"]) /
                       total_reps,
    P_ContinuousTA = as.numeric(win_tab["Continuous Transmission Acceleration"]) /
                       total_reps,
    P_OutbreakThreshold = as.numeric(win_tab["Outbreak Threshold"]) /
                            total_reps,
...
HH_ALPHA_BONF <- HH_ALPHA / length(DETECTOR_PAIRS)  # legacy reference
...
# ------------------------------------------------------------------------------
compute_detector_paired_wilcoxon <- function(metric_id, regional_metrics_df) {
...
build_regional_dominance_matrix <- function(metric_id) {
  ...
  ggplot2::ggplot(this_metric_df) +
    ...
    theme_dashboard(base_size = 8.5, base_family = base_family_global) +
    ggplot2::theme(
      ...
      plot.margin   = ggplot2::margin(40, 14, 6, 6)
    )
}
...
build_regional_dot_panel <- function(metric_id) {
  ...
  p <- p +
    ...
    theme_dashboard(base_size = 8.5, base_family = base_family_global) +
    ggplot2::theme(
      axis.text.x = ggplot2::element_text(size = 6.8, lineheight = 0.85,
                                          hjust = 0.5, vjust = 1,
                                          margin = ggplot2::margin(t = 2)),
      ...
      panel.grid.minor   = ggplot2::element_blank(),
      plot.margin = ggplot2::margin(6, 6, 22, 6)
    )
...
build_regional_sig_panel <- function(metric_id, all_wilcoxon_results) {
  ...
  ggplot2::ggplot(res, ggplot2::aes(y = Comparison, x = Abs_Median_Diff)) +
    ...
    ggplot2::geom_text(
      ...
      hjust = 0,
      family = base_family_global, size = 2.4, fontface = "bold",
    ...
    theme_dashboard(base_size = 8.0, base_family = base_family_global) +
    ggplot2::theme(
      axis.text.y  = ggplot2::element_text(size = 6.6, face = "bold",
                                           lineheight = 0.95),
      axis.text.x  = ggplot2::element_text(size = 6.5),
      axis.title.x = ggplot2::element_text(size = 7.0),
...
build_regional_multipanel <- function(metric_id) {
  ...
  (top / legend_strip / bottom) +
    ...
    patchwork::plot_annotation(
      title = paste0("Figure 4 | Regional generalisability: ",
...
build_method_summary_matrix <- function() {
  ...
  ggplot2::ggplot(df) +
    ...
    ggplot2::coord_cartesian(clip = "off") +
    ggplot2::labs(
      title = "a    Regional dominance matrix on 5 operational metrics",
      x = NULL, y = NULL
    ) +
    theme_dashboard(base_size = 8.5, base_family = base_family_global) +
    ggplot2::theme(
      ...
      panel.border  = ggplot2::element_blank(),
      legend.position = "none",
      plot.margin   = ggplot2::margin(20, 14, 6, 6)
    )
}
...
build_method_summary_dot <- function() {
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::labs(
      title = "b    Per-method sweep counts across the 5 metrics",
...
for (m in HH_METRICS_REGIONAL) {
  ...
  pdf_out <- file.path(OUTPUT_DIR, paste0("Figure4_", slug, ".pdf"))
  png_out <- file.path(OUTPUT_DIR, paste0("Figure4_", slug, ".png"))
...
cat("\n=== Building Method-Across-Metrics Summary figure (Figure 4 set) ===\n")
{
  ...
  pdf_out <- file.path(OUTPUT_DIR, "Figure4_Method_Summary.pdf")
  png_out <- file.path(OUTPUT_DIR, "Figure4_Method_Summary.png")
  ggplot2::ggsave(filename = pdf_out, plot = fig_summary,
                  device = safe_pdf_device(),
                  width = fig4_w, height = fig4_h,
                  units = "in", dpi = 600, limitsize = FALSE, bg = "white")
  ggplot2::ggsave(filename = png_out, plot = fig_summary, device = "png",
                  width = fig4_w, height = fig4_h,
                  units = "in", dpi = 600, limitsize = FALSE, bg = "white")
  cat("  Saved: ", pdf_out, "\n", sep = "")
  cat("  Saved: ", png_out, "\n", sep = "")
...
# columns). RFR / RFR_RX legacy columns dropped.
utils::write.csv(
...
get_ph_regions <- function() {
  if (!is.null(PH_SHAPEFILE) && file.exists(PH_SHAPEFILE)) {
    return(sf::st_read(PH_SHAPEFILE, quiet = TRUE))
  }
  if (requireNamespace("geodata", quietly = TRUE)) {
    g <- try(geodata::gadm(country = "PHL", level = 1, path = tempdir()),
             silent = TRUE)
    if (!inherits(g, "try-error") && !is.null(g)) return(sf::st_as_sf(g))
    g2 <- try(geodata::gadm(country = "PHL", level = 2, path = tempdir()),
              silent = TRUE)
    if (!inherits(g2, "try-error") && !is.null(g2)) return(sf::st_as_sf(g2))
  }
  if (requireNamespace("rnaturalearth", quietly = TRUE)) {
    g <- try(rnaturalearth::ne_states(country = "Philippines",
                                      returnclass = "sf"), silent = TRUE)
    if (!inherits(g, "try-error") && !is.null(g)) return(g)
  }
  stop("No Philippines geometry source available. Provide a shapefile via ",
       "PH_SHAPEFILE or install 'geodata' / 'rnaturalearth'.", call. = FALSE)
}
...
# ------------------------------------------------------------------------------
suppressWarnings({
  centroids_sf <- sf::st_point_on_surface(map_df)
  coords_xy    <- sf::st_coordinates(centroids_sf)
  label_df     <- sf::st_drop_geometry(centroids_sf) %>%
    dplyr::mutate(
      lon = coords_xy[, 1], lat = coords_xy[, 2],
      label = dplyr::case_when(
        consensus_tier == "strong" ~ paste0(
          REGION, "\n(",
          formatC(Dominance_Probability, format = "f", digits = 2), ")"),
        consensus_tier == "partial" ~ paste0(
          REGION, "\n(", Leader_Label_short, ", P=",
          formatC(Dominance_Probability, format = "f", digits = 2),
          ", partial)"),
        consensus_tier == "lead_only" ~ paste0(
          REGION, "\n(lead: ", Leader_Label_short, ", P=",
          formatC(Dominance_Probability, format = "f", digits = 2), ")"),
        consensus_tier == "contested" ~ paste0(
          REGION, "\n(contested, P=",
          formatC(Dominance_Probability, format = "f", digits = 2), ")"),
        TRUE ~ paste0(REGION, "\n(P=",
                      formatC(Dominance_Probability, format = "f", digits = 2),
...
p_map <- ggplot2::ggplot(map_df) +
  ggplot2::geom_sf(
    ggplot2::aes(fill = Tier_Fill_Group, alpha = Confidence_Tier),
    colour = "grey25",
    linetype = map_df$Tier_Linetype,
    linewidth = map_df$Tier_Linewidth
  ) +
  ggplot2::geom_point(
    data = label_df, ggplot2::aes(x = lon, y = lat),
    size = 0.7, colour = "grey15",
    inherit.aes = FALSE, show.legend = FALSE
  ) +
  ggrepel::geom_label_repel(
    data = label_df, ggplot2::aes(x = lon, y = lat, label = label),
    size = 2.3, fontface = "bold", fill = "white", colour = "grey15",
    label.size = 0.30,
    ...
    segment.size = 0.30, segment.alpha = 0.95, seed = 12345,
    lineheight = 0.85,
    inherit.aes = FALSE, show.legend = FALSE
  ) +
  ggplot2::scale_fill_manual(
    name = "Dominant detector",
    values = tier_fill_palette, drop = TRUE
  ) +
  ggplot2::scale_alpha_manual(
    name = "Bootstrap confidence  (color shade)",
    values = alpha_palette, drop = TRUE
  ) +
  ggplot2::guides(
    fill  = ggplot2::guide_legend(order = 1, title.position = "top",
                                  title.hjust = 0.5, nrow = 1, byrow = TRUE,
                                  override.aes = list(alpha = 0.9)),
    alpha = ggplot2::guide_legend(order = 2, title.position = "top",
                                  title.hjust = 0.5, nrow = 1, byrow = TRUE,
                                  keywidth  = grid::unit(0.9, "lines"),
                                  keyheight = grid::unit(0.9, "lines"))
  ) +
  ggplot2::labs(
    title = "a   Regional dominance of the leading detector  (consensus winner)"
  ) +
  ggplot2::coord_sf(clip = "off", expand = TRUE) +
  ...
  ggplot2::theme(
    ...
    plot.margin    = ggplot2::margin(15, 25, 5, 15)
  )
...
p_table <- ggplot2::ggplot() +
  ...
  ggplot2::scale_x_continuous(
    ...
    limits = c(0.5, N_COLS + 0.5), expand = c(0, 0)
  ) +
  ggplot2::scale_y_continuous(
    limits = c(y_min_tbl, y_max_tbl), expand = c(0, 0)
  ) +
  ggplot2::labs(
...
panel_c_long <- dominance_df_canonical %>%
  dplyr::select(REGION, P_ConstantTA, P_ContinuousTA, P_OutbreakThreshold) %>%
  tidyr::pivot_longer(cols = c(P_ConstantTA, P_ContinuousTA, P_OutbreakThreshold),
                      names_to = "Detector_Code", values_to = "Dominance_P") %>%
  dplyr::mutate(
    Detector_Label = dplyr::case_when(
      Detector_Code == "P_ConstantTA"        ~ "Constant TA",
      Detector_Code == "P_ContinuousTA"      ~ "Continuous TA",
      Detector_Code == "P_OutbreakThreshold" ~ "Outbreak Threshold"
    ),
    Detector_Label = factor(Detector_Label,
                            levels = c("Constant TA", "Continuous TA",
                                       "Outbreak Threshold"))
  ) %>%
  dplyr::left_join(
    winner_rows %>%
  ...
  dplyr::mutate(
    is_leader_dot = dplyr::case_when(
      Detector_Label == "Constant TA"        & Observed_Winner ==
        "Constant Transmission Acceleration"   ~ TRUE,
      Detector_Label == "Continuous TA"      & Observed_Winner ==
        "Continuous Transmission Acceleration" ~ TRUE,
      Detector_Label == "Outbreak Threshold" & Observed_Winner ==
        "Outbreak Threshold"                   ~ TRUE,
      TRUE ~ FALSE
    ),
...
p_dot <- ggplot2::ggplot(panel_c_long,
  ...
  ggplot2::annotate("rect", xmin = -Inf, xmax = Inf,
                    ymin = DOMINANCE_THRESHOLD, ymax = 1.0,
                    fill = "#cfe2f3", alpha = 0.18) +
  ggplot2::geom_hline(yintercept = chance_p, linetype = "dashed",
                      linewidth = 0.35, colour = "grey55") +
  ggplot2::geom_hline(yintercept = DOMINANCE_THRESHOLD, linetype = "dashed",
                      linewidth = 0.35, colour = "grey55") +
  ggplot2::annotate("text", x = 0.55, y = chance_p, label = "chance (1/3)",
                    hjust = 0, vjust = -0.5, size = 2.9,
                    family = base_family_global, colour = "grey45",
                    fontface = "italic") +
  ggplot2::annotate("text", x = 0.55, y = DOMINANCE_THRESHOLD,
                    label = "decisive",
                    hjust = 0, vjust = -0.5, size = 2.9,
                    family = base_family_global, colour = "grey45",
                    fontface = "italic") +
  ...
  ggplot2::labs(
    title = "c   Per-detector regional dominance probabilities",
...
# ------------------------------------------------------------------------------
top_row <- (p_map | p_table) + patchwork::plot_layout(widths = c(1.0, 2.0))
bottom_row <- (patchwork::plot_spacer() | p_dot |
                 patchwork::plot_spacer()) +
  patchwork::plot_layout(widths = c(0.5, 2.0, 0.5))

fig5 <- (top_row / bottom_row) +
  patchwork::plot_layout(heights = c(2.4, 1.4)) +
  patchwork::plot_annotation(
    title = "Figure 5 | Bootstrap-supported regional dominance of three target detectors",
    theme = ggplot2::theme(
...
FIG5_W <- 20
FIG5_H <- 13
ggplot2::ggsave(file.path(OUTPUT_DIR, "Figure5_Detector_Map.pdf"),
                fig5, width = FIG5_W, height = FIG5_H,
                device = safe_pdf_device(),
                units = "in", limitsize = FALSE, bg = "white")
ggplot2::ggsave(file.path(OUTPUT_DIR, "Figure5_Detector_Map.png"),
                fig5, width = FIG5_W, height = FIG5_H, dpi = 600,
                device = "png",
                units = "in", limitsize = FALSE, bg = "white")
cat("\nSaved: ", file.path(OUTPUT_DIR, "Figure5_Detector_Map.pdf"), "\n",
    sep = "")
cat("Saved: ", file.path(OUTPUT_DIR, "Figure5_Detector_Map.png"), "\n",
    sep = "")
...
#   Stage4_Figure4_Legend_Drafts.txt   (one block per metric + method summary)
#   Stage4_Figure5_Legend_Draft.txt    (single block for the detector map figure)
# ------------------------------------------------------------------------------
...
# Combined Figure 4 legend draft (one block per metric + Method Summary).
fig4_legend_lines <- c(
  paste0("================================================================"),
  paste0("FIGURE 4  |  LEGEND DRAFTS  (per-metric and method summary)"),
  paste0("================================================================")
)
for (m in HH_METRICS_REGIONAL) {
  fig4_legend_lines <- c(
...
writeLines(fig4_legend_text,
           file.path(OUTPUT_DIR, "Stage4_Figure4_Legend_Drafts.txt"))
cat("\nSaved: ", file.path(OUTPUT_DIR, "Stage4_Figure4_Legend_Drafts.txt"),
    "\n", sep = "")

# Figure 5 legend draft
build_figure5_legend <- function() {
  ...
  paste0(
    ...
    "the Philippines. The figure has three panels: (a) a choropleth map ",
    "encoding the four-tier regional consensus, (b) a per-region metric ",
    "table with embedded per-metric significance, and (c) a per-detector ",
    "dot plot of regional dominance probabilities. The per-region per-",
    "metric significance results are also exported as a standalone CSV ",
    "(Stage4_Detector_Map_PanelD_HeatmapData.csv) for downstream reporting.\n\n",
    ...
    "Dashed lines mark chance (1/3) and decisive (",
...
writeLines(fig5_legend_text,
           file.path(OUTPUT_DIR, "Stage4_Figure5_Legend_Draft.txt"))
cat("Saved: ", file.path(OUTPUT_DIR, "Stage4_Figure5_Legend_Draft.txt"),
    "\n", sep = "")

# Echo legend drafts to console.
cat("\n", fig4_legend_text, "\n", sep = "")
cat("\n", fig5_legend_text, "\n", sep = "")
...
cat("  Figure5_Detector_Map.{pdf,png}\n")
...
cat("Legend drafts:\n")
cat("  Stage4_Figure4_Legend_Drafts.txt\n")
cat("  Stage4_Figure5_Legend_Draft.txt\n")
